...
return;$p=$e.data(key);$e.insertAfter($p);$e.removeData(key)}})}})(jQuery);(function($){skel.breakpoints({xlarge:'(max-width: 1680px)',large:'(max-width: 1280px)',medium:'(max-width: 1024px)',small:'(max-width: 736px)',xsmall:'(max-width: 480px)'});$(function(){var $body=$('body'),$header=$('#header'),$nav=$('#nav'),$nav_a=$nav.find('a'),$wrapper=$('#wrapper');$('form').placeholder();skel.on('+medium -medium',function(){$.prioritize('.important\\28 medium\\29',skel.breakpoint('medium').active)});var ids=[];$nav_a.scrolly({offset:44}).on('click',function(event){var $this=$(this),href=$this.attr('href');if(href.charAt(0)!='#')
return;event.preventDefault();$nav_a.removeClass('active').addClass('scrollzer-locked');$this.addClass('active')}).each(function(){var $this=$(this),href=$this.attr('href'),id;if(href.charAt(0)!='#')
return;id=href.substring(1);$this.attr('id',id+'-link');ids.push(id)});$.scrollzer(ids,{pad:300,lastHack:!0});$('<div id="titleBar">'+'<a href="#header" class="toggle"></a>'+'<span class="title">'+$('#logo').html()+'</span>'+'</div>').appendTo($body);$('#header').panel({delay:500,hideOnClick:!0,hideOnSwipe:!0,resetScroll:!0,resetForms:!0,side:'right',target:$body,visibleClass:'header-visible'});if(skel.vars.os=='wp'&&skel.vars.osVersion<10)
$('#titleBar, #header, #wrapper').css('transition','none')})})(jQuery);url1="https://api.github.com/repos/sharadbhat/Music-Streaming-Service";url2="https://api.github.com/repos/sharadbhat/nodorithm";url3="https://api.github.com/repos/sharadbhat/VideoHub";url4="https://api.github.com/repos/sharadbhat/Zomatopy";url5="https://api.github.com/repos/sharadbhat/Recommender-Engine-API";url6="https://api.github.com/repos/OmkarPathak/pygorithm";url=[url1,url2,url3,url4,url5,url6];var star='star'
var fork='fork'
var xhr=new XMLHttpRequest();for(var i=1;i<url.length+1;i++){xhr.open("GET",url[i-1],!1);xhr.send();json=JSON.parse(xhr.responseText);var repoStars=json.stargazers_count;var repoForks=json.forks_count;document.getElementById(star+i.toString()).innerHTML=repoStars;document.getElementById(fork+i.toString()).innerHTML=repoForks}
function myMapp(){var myCenter=new google.maps.LatLng(12.8614515,77.66470809999998);var mapOptions={center:myCenter,zoom:15,mapTypeId:google.maps.MapTypeId.MAP}
var marker=new google.maps.Marker({position:myCenter});var map=new google.maps.Map(document.getElementById("map"),mapOptions);marker.setMap(map)}(function($){"use strict";$(window).load(function(){$("#loader").fadeOut("slow",function(){$("#preloader").delay(300).fadeOut("slow")})})
setTimeout(function(){$('.main-content h1').fitText(.8,{minFontSize:'42px',maxFontSize:'94px'})},100);$(".kern-this").lettering();var toggleButton=$('.menu-toggle'),nav=$('#menu-nav-wrap'),mainContent=$('#main-404-content'),mainHeader=$('.main-header');$('.main-content-particle-js').particleground({dotColor:'#fff',lineColor:'#555555',particleRadius:6,curveLines:!0,density:9000,proximity:100})})(jQuery)
